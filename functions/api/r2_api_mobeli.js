export async function onRequest(context) {
  ...
  try {
    if (request.method === "GET") {
      const list = await env.MOBELI_BUCKET.list({ prefix: prefix, limit: 1000 });
      const files = list.objects.map(obj => ({
        key: obj.key,
        url: `https://file.mobeli.my.id/${obj.key}`,
        size: obj.size,
    ...
    if (request.method === "POST") {
      ...
      if (action === "upload") {
        ...
        await env.MOBELI_BUCKET.put(fileName, file.stream(), {
          httpMetadata: {
        ...
        return new Response(JSON.stringify({
          success: true,
          url: `https://file.mobeli.my.id/${fileName}`
        }), { headers: { "Content-Type": "application/json", ...corsHeaders } });
      }

      if (action === "rename") {
        const { oldKey, newKey, keepOriginal } = await request.json();
        const obj = await env.MOBELI_BUCKET.get(oldKey);
        if (!obj) return new Response(JSON.stringify({ error: "File not found" }), { status: 404, headers: corsHeaders });
        await env.MOBELI_BUCKET.put(newKey, obj.body, { httpMetadata: obj.httpMetadata });
        if (!keepOriginal) await env.MOBELI_BUCKET.delete(oldKey);
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }

      if (action === "delete") {
        const { key } = await request.json();
        await env.MOBELI_BUCKET.delete(key);
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }
    }
  } catch (e) {
